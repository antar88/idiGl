...
static Point translateCamVector = Point (0,0,0);
...
void updateCamera(float x, float y) {
    ...
    if(camMov == CAM_MOVEMENT_LOOKAT){
        gluLookAt(obsPos.x+translateCamVector.x, obsPos.y+translateCamVector.y, obsPos.z+translateCamVector.z, translateCamVector.x, translateCamVector.y, translateCamVector.z, up.x, up.y, up.z);
        GLdouble m[16];
    ...
    else if(camMov == CAM_MOVEMENT_EULER) {
