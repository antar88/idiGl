...
//Cams
static Cam axoCam = Cam(CAM_MOVEMENT_LOOKAT,Point(0.0,0.0,2), Point(0.f,1.f,0.f), Point (0,0,0), Point (0,0,0), Point (0,0,0));
static Cam prespectiveCam = Cam(CAM_MOVEMENT_LOOKAT, Point (0,1,0), Point (0,0,0), contSphere, SCREEN_W, SCREEN_H);
...
void firstPaint(void)
{
    ...
    axoCam.updateCamType(aWidth, aheigth);
    axoCam.updateCamPos();
    
    prespectiveCam.updateCamType(aWidth, aheigth);
    prespectiveCam.updateCamPos();
...
void reshape(int x, int y) {
    glViewport(0,0,x,y);
    if (cam == AXONOMETRIC_CAM) axoCam.updateCamType(aWidth, aheigth);
    else prespectiveCam.updateCamType(aWidth, aheigth);
    glutPostRedisplay();
    aWidth = x;
    aheigth = y;
}
...
void paintModeCam(int paintMode) {
    if(cam == AXONOMETRIC_CAM) axoCam.changePaintMode(paintMode);
    else prespectiveCam.changePaintMode(paintMode);
}
    ...
    switch (mode) {
        case ZOOM_MODE:
            ...
            if(zoomMode == CAM_ZOOM_MOVEMENT) {
                if(cam == AXONOMETRIC_CAM) cerr << "Axonometric camera with fovy change zoom type has no effect" << endl;
                else moveCam(Point(0.0,0.0,-z));
            }
            else zoomCam(z);
            break;
        case PAN_MODE:
...
void keyboardInteraction(unsigned char key, int xMouse, int yMouse) {
    ...
    switch (key) {
        ...
        case '1':
            ...
            prespectiveCam = Cam(CAM_MOVEMENT_LOOKAT, Point(0.0,1.0,0.0), Point (0,0,0), contSphere, SCREEN_W, SCREEN_H);
            updateCam = true;
            break;
        case '2':
            ...
            prespectiveCam = Cam(CAM_MOVEMENT_LOOKAT, Point(0.0,0.0,-1.0), Point (90.0,0.0,0.0), contSphere, SCREEN_W, SCREEN_H);
            updateCam = true;
            break;
        case '3':
            cout << "Changed cam position 3(X,X,X)" << endl;
            axoCam = Cam(CAM_MOVEMENT_LOOKAT,Point(1.0,1.0,1.0), Point(0,1,0), Point (0,0,0), Point (0,0,0), Point (0,0,0));
            prespectiveCam = Cam(CAM_MOVEMENT_LOOKAT, Point(0,1,0), Point (45.0,45.0,0.0), contSphere, SCREEN_W, SCREEN_H);
            updateCam = true;
            break;
        default:
            refresh = false;
